...
#define HID_CONFIG_TOTAL_LEN (TUD_CONFIG_DESC_LEN + 2 * TUD_HID_DESC_LEN)
...
static const uint8_t HID_CONFIGURATION_DESCRIPTOR[] = {
    ...
    TUD_HID_DESCRIPTOR(HID_ITF_MOUSE, 4, HID_ITF_PROTOCOL_MOUSE,
...
{
    return instance == HID_ITF_KEYBOARD ? HID_KEYBOARD_REPORT_DESCRIPTOR : HID_MOUSE_REPORT_DESCRIPTOR;
}
...
static void hid_event_cb(tinyusb_event_t *event, void *arg)
{
    ...
    case TINYUSB_EVENT_ATTACHED:
        ESP_LOGI(TAG, "USB HID mounted by host");
        break;
...
{
    return s_installed && tud_mounted();
}
...
static void send_mouse_counts(int dx, int dy, uint32_t timeout_ms)
{
    ...
    send_mouse_report(clamp_i8(dx), clamp_i8(dy), 0, 0, timeout_ms);
}
...
{
    ESP_LOGI(TAG, "Installing TinyUSB HID device");
    tinyusb_config_t tusb_cfg = TINYUSB_DEFAULT_CONFIG();
    tusb_cfg.descriptor.device = NULL;
    tusb_cfg.descriptor.full_speed_config = HID_CONFIGURATION_DESCRIPTOR;
    tusb_cfg.descriptor.string = HID_STRING_DESCRIPTOR;
    tusb_cfg.descriptor.string_count = sizeof(HID_STRING_DESCRIPTOR) / sizeof(HID_STRING_DESCRIPTOR[0]);
    tusb_cfg.event_cb = hid_event_cb;
#if TUD_OPT_HIGH_SPEED
    tusb_cfg.descriptor.high_speed_config = HID_CONFIGURATION_DESCRIPTOR;
#endif

    ESP_RETURN_ON_ERROR(tinyusb_driver_install(&tusb_cfg), TAG, "install tinyusb");
    s_installed = true;
    ESP_LOGI(TAG, "TinyUSB HID installed");
    return ESP_OK;
}
...
{
    send_mouse_report(0, 0, delta_y, delta_x, 20);
}
