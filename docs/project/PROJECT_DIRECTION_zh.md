...
- `exoanchor-agent/`：服务端 Agent runtime，后续接入设备层，负责观察、动作、安全确认和日志。
...
4. Agent 层先做服务端持久化 runtime 和结构化 observation，再增强自主规划。
...
- Agent 层可审计、可恢复，负责高层决策、SSH/KVM 通道选择、安全确认、日志和 playbook。
