...
class LLMClient:
    ...
    @staticmethod
    def _openai_compatible_endpoint(provider: str, nlp_cfg: dict[str, Any]) -> str:
        endpoint = str(nlp_cfg.get("endpoint") or "").strip()
        ...
        return endpoint.rstrip("/") + "/v1/chat/completions" if not endpoint.rstrip("/").endswith("/v1") else endpoint.rstrip("/") + "/chat/completions"
    ...
    async def complete(
        ...
        *,
        ...
        if provider in ("openai", "custom", "deepseek"):
            endpoint = self._openai_compatible_endpoint(provider, nlp_cfg)
            if provider == "deepseek":
            ...
            reasoning_effort = nlp_cfg.get("reasoning_effort")
            if reasoning_effort:
                payload["reasoning_effort"] = reasoning_effort
            thinking = nlp_cfg.get("thinking")
