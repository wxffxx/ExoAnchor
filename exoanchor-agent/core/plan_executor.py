...
class PlanExecutor:
    ...
    async def resume_saved_run(self, run_id: str) -> PlanRunStatus:
        async with self._lock:
            ...
            for idx, step in enumerate(resumed.steps):
                ...
                step.policy_action = None
                step.policy_risk_level = None
                step.policy_reason = None
                step.policy_rules = []
    ...
    def _evaluate_policy(self, run: PlanRunStatus, step: PlanStepStatus) -> PolicyDecision:
        if self.policy is None:
            decision = PolicyDecision(tool_name=step.tool, command=step.command, args=step.args)
            self._apply_policy_decision(step, decision)
            return decision
    ...
    def _apply_policy_decision(self, step: PlanStepStatus, decision: PolicyDecision) -> None:
        step.policy_action = decision.action.value if hasattr(decision.action, "value") else str(decision.action)
        step.policy_risk_level = decision.risk_level.value if hasattr(decision.risk_level, "value") else str(decision.risk_level)
        step.policy_reason = decision.reason or ""
        step.policy_rules = list(decision.matched_rules or [])
