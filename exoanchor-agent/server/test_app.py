...
def create_test_app(project_root: str | None = None) -> FastAPI:
    ...
    config_path = os.environ.get(
        "EXOANCHOR_CONFIG_PATH",
        os.path.join(project_root, ".local", "exoanchor_config.json"),
    )
    config_store = JSONConfigStore(
        primary_path=config_path,
        legacy_paths=[os.path.join(project_root, "exoanchor_config.json")],
    )
    ...
    async def _llm_models_response(provider: str = "gemini", api_key: str = "", endpoint: str = ""):
    ...
    async def proxy_llm_models(provider: str = "gemini", api_key: str = "", endpoint: str = ""):
        return await _llm_models_response(provider=provider, api_key=api_key, endpoint=endpoint)

    @app.post("/api/llm/models")
    async def proxy_llm_models_post(request: Request):
        body = await request.json()
        return await _llm_models_response(
