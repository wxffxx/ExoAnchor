...
KNOWN_DEEPSEEK_MODELS = [
    {"id": "deepseek-v4-pro", "name": "DeepSeek V4 Pro", "desc": "Reasoning / hard system-management tasks"},
    {"id": "deepseek-v4-flash", "name": "DeepSeek V4 Flash", "desc": "Fast and cost-effective planning"},
]
...
async def fetch_provider_models(provider: str = "gemini", api_key: str = "", endpoint: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        if provider == "deepseek":
            ...
            base = (endpoint or "https://api.deepseek.com").rstrip("/")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    ...
                    if "error" in data:
                        error_message = data["error"].get("message", "Unknown error")
                        return {"models": KNOWN_DEEPSEEK_MODELS, "error": error_message}
                    models = [{"id": model["id"], "name": model["id"], "desc": ""} for model in data.get("data", [])]
                    return {"models": models or KNOWN_DEEPSEEK_MODELS}
