...
- the local dev server used to depend on a root-level launcher
- the web UI has historically owned too much execution state
- CLI and dashboard did not share one runtime event contract
- historical workload/RCON helpers and product-specific patches polluted the core runtime
...
- domain behaviors like Minecraft admin become optional skills or plugin modules, not core runtime shortcuts
...
- keep product/domain resolution outside the core runtime
