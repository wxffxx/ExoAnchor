...
static esp_err_t execute_command(const si_hid_command_t *command,
                                 const si_hid_owner_token_t *owner,
                                 si_hid_authority_guard_fn guard,
                                 void *guard_context)
{
    return si_hid_execute_owned(command, owner, guard, guard_context);
}

static esp_err_t hid_json_execute_internal(const cJSON *message,
                                           const si_hid_owner_token_t *owner,
                                           si_hid_authority_guard_fn guard,
                                           void *guard_context)
{
    ...
    if (!cJSON_IsObject(message)) {
        command.error = "invalid hid json";
        return execute_command(&command, owner, guard, guard_context);
    }
                                    ...
                                    void *guard_context)
{
    ...
    return hid_json_execute_internal(message, owner, guard, guard_context);
}
