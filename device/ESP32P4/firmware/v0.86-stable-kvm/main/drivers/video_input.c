...
static void yuy2_encode_task(void *arg)
{
    ...
    while (true) {
        ...
        while (true) {
            ...
            uint32_t quality = SI_CFG_VIDEO_JPEG_QUALITY;
...
{
    s_latest_jpeg_cap = SI_CFG_VIDEO_JPEG_BUFFER_SIZE;
...
esp_err_t si_video_copy_jpeg(uint8_t *buf, size_t buf_cap, size_t *out_len, uint32_t *out_frame_id)
{
    ...
    *out_len = 0;
    ...
    if (s_status.last_jpeg_size > buf_cap) {
        xSemaphoreGive(s_lock);
        return ESP_ERR_INVALID_SIZE;
    }

    memcpy(buf, s_latest_jpeg, s_status.last_jpeg_size);
...
esp_err_t si_video_acquire_jpeg(uint8_t **out_buf, size_t *out_len, uint32_t *out_frame_id)
{
    ...
    *out_len = 0;
    ...
    for (int attempt = 0; attempt < 2; attempt++) {
        ...
        esp_err_t ret = si_video_copy_jpeg(copy, need, out_len, out_frame_id);
